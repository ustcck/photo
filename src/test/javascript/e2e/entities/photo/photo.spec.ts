...
import PhotoDetailsPage from './photo-details.page-object';
...
describe('Photo e2e test', () => {
  ...
  let updatePage: PhotoUpdatePage;
  let detailsPage: PhotoDetailsPage;
  let listPage: PhotoComponentsPage;
  let deleteDialog: PhotoDeleteDialog;
  ...
  it('should load Photos', async () => {
    ...
    listPage = new PhotoComponentsPage();
  ...
  describe('Create flow', () => {
    it('should load create Photo page', async () => {
      ...
      updatePage = new PhotoUpdatePage();

      await waitUntilAllDisplayed([updatePage.title, updatePage.footer, updatePage.saveButton]);

      expect(await updatePage.title.getAttribute('id')).to.match(/photoApp.photo.home.createOrEditLabel/);
    });
    ...
    describe('Details, Update, Delete flow', () => {
      after(async () => {
        ...
        deleteDialog = new PhotoDeleteDialog();
        await waitUntilDisplayed(deleteDialog.dialog);

        expect(await deleteDialog.title.getAttribute('id')).to.match(/photoApp.photo.delete.question/);
      ...
      it('should load details Photo page and fetch data', async () => {
        const detailsButton = listPage.getDetailsButton(listPage.records.last());
        await click(detailsButton);

        detailsPage = new PhotoDetailsPage();
      ...
      it('should load edit Photo page, fetch data and update', async () => {
